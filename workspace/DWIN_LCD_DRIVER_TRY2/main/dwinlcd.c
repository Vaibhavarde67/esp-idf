...
//			ets_printf("Received VP Address = %x\n",recevied_dwin_frametemp->VP_addr);
			if(uxQueueSpacesAvailable(dwin_frame_queu))
			{
				BaseType_t xHigherPriorityTaskWoken;
				// We have not woken a task at the start of the ISR.
				xHigherPriorityTaskWoken = pdFALSE;

//				xQueueSendToBackFromISR( dwin_frame_queu, &recevied_dwin_frametemp, &xHigherPriorityTaskWoken );
	...
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.at_cmd_char_det);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.brk_det);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.cts_chg);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.dsr_chg);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.frm_err);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.glitch_det);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.parity_err);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.rxfifo_full);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.rxfifo_ovf);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.rxfifo_tout);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.sw_xoff);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.sw_xon);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.tx_brk_done);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.tx_brk_idle_done);
	ets_printf("%d-",DWIN_LCD_UART_No.int_st.tx_done);
	ets_printf("%d-\n",DWIN_LCD_UART_No.int_st.txfifo_empty);
	ets_printf("%d\n",DWIN_LCD_UART_No.int_st.val);
	...
	uart_clear_intr_status(DWIN_LCD_UART, (UART_RXFIFO_FULL_INT_CLR | UART_RXFIFO_OVF_INT_CLR | UART_RXFIFO_TOUT_INT_CLR | UART_TXFIFO_EMPTY_INT_CLR));
	//uart_clear_intr_status(DWIN_LCD_UART, 0x00);
}
...
//	uart_disable_intr_mask(DWIN_LCD_UART, DWIN_INTR_UART_CONFIG_FLAG);
	uart_isr_register(DWIN_LCD_UART,mod_uart_intr_handle, 0, ESP_INTR_FLAG_IRAM, &handle_console);


	//enable RX interrupt
	//uart_enable_rx_intr(DWIN_LCD_UART);
	//uart_disable_tx_intr(DWIN_LCD_UART);
	uart_disable_intr_mask(DWIN_LCD_UART, 0x7FF);
//	uart_disable_intr_mask(DWIN_LCD_UART, (UART_TX_DONE_INT_ENA_M | UART_TXFIFO_EMPTY_INT_RAW));
