...
static const char *TAG = "DISPLAY FUNCTION";
 ...
 void IRAM_ATTR disp_scroll_fun(void )
 {
 ...
 //					Pixel_Data2[l][i][j] |= (Pixel_Data2[l][i+3][j]>>15);
 					Pixel_Data2[l][i][j] = (Pixel_Data2[l][i][j]*2);
 					Pixel_Data2[l][i][j] |= (Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j]/32768);
 					Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j] = Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j]*2;
 //					Pixel_Data2[l][i][j] = (Pixel_Data2[l][i][j]<<1);
 //					Pixel_Data2[l][i][j] |= (Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j]>>15);
 //					Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j] = Pixel_Data2[l][i+3+crnt_col_sel[selc_row]][j]<<1;
 				}
 				column_cn[selc_row]++;

 				if(column_cn[selc_row]==16)
 				{
 					column_cn[selc_row] = 0;
 					crnt_col_sel[selc_row]++;
 				}
 //				i +=3;
 				//for buffers
 //				end_pos = MAX_LINE_VERT_REG-1;
...
// 		selc_row = 0;
 	...
 	for(selc_row = 0; selc_row!=no_row; selc_row++)
 	{
 	...
 	{
 		if(display_prograns[current_program].prg_lines[selc_row].cur_scr_pos == (max_scr_position_disp-32))
		{
			for(uint8_t j =0; j< no_row; j++)
			{
				display_prograns[current_program].prg_lines[j].cur_scr_pos = 0;
				//ets_printf("Vaibhav Here\n");
				for(uint8_t v=0; v!=MAX_LINE_VERT_REG; v++)
				{
					for(uint8_t b=0 ; b != 16 ; b++)                   //This is because we have 16bit data
					{
						Pixel_Data2[j][v][b] = Pixel_Data[j][v][b];
					}
				}
			}
		}
...
//		rem = display_prograns[current_program].prg_lines[n].cur_scr_pos % 16;
		for(j=0; j!= no_of_vert_reg2; j++)    //This loop is for shift register columns
		{
			...
			for(m=1; m != -1 ; m--)                   //This is because we have 16bit data
			{
				l=15-i;
				for(k=3 ; k != -1; k--)           //This is for 1 74595 Column
				{
					//
					if(m)
					{
...
//	ledc_stop(INTENSITY_HS_MODE, INTENSITY_HS_CH0_CHANNEL, 0);
	//ledc_set_duty(INTENSITY_HS_MODE,INTENSITY_HS_CH0_CHANNEL, 255);
