...
esp_err_t esp_modem_dce_connect_gprs(modem_dce_t *dce)
{
	modem_dte_t *dte = dce->dte;
	char command[64];
	int len = snprintf(command, sizeof(command), "AT+CGATT=%d\r", 1);
	DCE_CHECK(len < sizeof(command), "command too long: %s", err, command);
	dce->handle_line = esp_modem_dce_handle_response_default;
	DCE_CHECK(dte->send_cmd(dte, command, MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", err);
	DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "define pdp context failed", err);
...
esp_err_t esp_modem_dce_disconnect_gprs(modem_dce_t *dce)
{
	modem_dte_t *dte = dce->dte;
	char command[64];
	int len = snprintf(command, sizeof(command), "AT+CGATT=%d\r", 0);
	DCE_CHECK(len < sizeof(command), "command too long: %s", err, command);
	dce->handle_line = esp_modem_dce_handle_response_default;
	DCE_CHECK(dte->send_cmd(dte, command, MODEM_COMMAND_TIMEOUT_DEFAULT) == ESP_OK, "send command failed", err);
	DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "define pdp context failed", err);
