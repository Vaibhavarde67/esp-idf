...
modem_dce_t *sim800_init(modem_dte_t *dte)
{
    ...
    /*
     * Checking wethere SIM800 module is connected or not
     * It also check wether it is sending responce back or not by using AT command
     */
    if(esp_modem_dce_sync(&(esp_modem_dce->parent)) == ESP_FAIL)
    {
    	ESP_LOGE(DCE_TAG, "SIM800 is not Responding");
    	goto err_io;
    }
    /* Close echo */
    //DCE_CHECK(esp_modem_dce_echo(&(esp_modem_dce->parent), false) == ESP_OK, "close echo mode failed", err_io);
    /*
     * Closing Echo Mode of SIM800
     */
    esp_modem_dce_echo(&(esp_modem_dce->parent), false);
    /* Get Module name */
    //DCE_CHECK(esp_modem_dce_get_module_name(&(esp_modem_dce->parent)) == ESP_OK, "get module name failed", err_io);
    /*
     * Getting Module name using AT command
     */
    esp_modem_dce_get_module_name(&(esp_modem_dce->parent));
    /* Get IMEI number */
    //DCE_CHECK(esp_modem_dce_get_imei_number(&(esp_modem_dce->parent)) == ESP_OK, "get imei failed", err_io);
    /*
     * Get IMEI Number of SIM800 Module
     */
    esp_modem_dce_get_imei_number(&(esp_modem_dce->parent));
    /* Get IMSI number */
    //DCE_CHECK(esp_modem_dce_get_imsi_number(&(esp_modem_dce->parent)) == ESP_OK, "get imsi failed", err_io);
    /*
     * Get IMSI Number of SIM inserted in SIM800 module
     */
    esp_modem_dce_get_imsi_number(&(esp_modem_dce->parent));
     ...
     * Get Operator Name
     */
    esp_modem_dce_get_operator_name(&(esp_modem_dce->parent));
