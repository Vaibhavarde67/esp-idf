	...
	if(prgm->prg_font == EnglishBig)
	{
		...
		for(uint8_t j =0; j< no_row; j++)
		{
			...
			else
			{
				...
				//sprintf(route_no_str, "%d", prgm->prg_lines[j].route_no);
...
//					display_ln[j][g] = 0x34;
			...
			if(dis_size > max_display_size)
			{
				...
				if(max_scr_position_disp < prgm->prg_lines[j].end_pos)
						max_scr_position_disp = prgm->prg_lines[j].end_pos;
...
void load_dummy_data(void)
{
	...
	for(uint8_t i=0; i<14; i++)
		...
		{
			display_prograns[i].prg_lines[j].type = simple;
